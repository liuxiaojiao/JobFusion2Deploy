...
#     loader = CSVLoader(file_path='course_inventory.csv', encoding="latin")
#     data = loader.load()

#     vectordb = build_vectordb(data)

#     user_profile = pd.read_csv('user_profile.csv', encoding="latin")
#     results = get_recommendations(data, vectordb, 10, user_profile)['result']
#     res = json.loads(results)
...
#         df_temp = pd.DataFrame.from_dict(k, orient='index').T
#         df = pd.concat([df, df_temp])
#     df.to_excel('output/deeplearning_course_recommendation_0619.xlsx', index=False)
